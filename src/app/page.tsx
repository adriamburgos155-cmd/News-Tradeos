...
export default function Terminal() {
  const { market: rawMarket, isMock, refresh, updatedAt } = useMarket()
    ...
    <div className="h-screen flex flex-col bg-black text-on-surface overflow-hidden">
      ...
      <div className="flex flex-1 min-h-0 pt-10 pb-6">
        ...
        <main className="flex-1 min-w-0 ml-12 md:ml-44 overflow-hidden px-3 pt-3 pb-1">
          ...
          {page === 'calendar' && (
            <CalendarView events={events} source={source} />
          )}
          {page === 'heatmap' && (
            <HeatmapView />
          )}
      ...
      {!chatOpen && (
        ...
        >
          <span className="w-1.5 h-1.5 rounded-full bg-on-primary live-dot" />
          AI_ANALYST
        </button>
      )}
